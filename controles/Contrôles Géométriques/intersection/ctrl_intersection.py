...
def ctrl_intersection(param, data, otherdata):
    ...
    for i in range(len(data) - 1):
        for j in range(len(otherdata) - 1):
            ...
            if ((point != None and point not in controles) and 
                (data[i] not in start_points and data[i + 1] not in start_points) and
                (otherdata[j] not in start_points and otherdata[j + 1] not in start_points) and 
                (param[0] != param[1] or None in param)):
