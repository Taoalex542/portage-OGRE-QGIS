...
class gestion_couches(QDockWidget):
    ...
    def add_layers(self):
        ...
        for i in allLayers:
            if i.name() == "zone_de_réconciliation" or i.name() == self.main.zone_plugin:
                continue
    ...
    # Gestion des couches pour les secondes demandes
    
    def precis_get_active_layers(self):
        self.main.couches_actives = 0
        for items in self.main.couche_list:
            if items[2] == QtCore.Qt.Checked:
                self.main.couches_actives += 1
        ...
        for items in self.main.couche_list:
            ...
            for i in range(root.childCount()):
                ...
                if (signal.text(0) == items[0]):
                    signal.setCheckState(0, items[2])
                if (num_children != 0):
                    self.precis_global_couche_prep(num_children, signal)
        self.main.dlg_precis.show()
        self.main.dlg_precis.treeWidget.expandAll()
    def precis_global_couche_prep(self, num_children, parent):
        for items in self.main.couche_list:
            for i in range(parent.childCount()):
                child = parent.child(i)
                num_children = child.childCount()
                if (child.text(0) == items[0]):
                    child.setCheckState(0, items[2])
                if num_children != 0:
                    self.precis_global_couche_prep(num_children, child)

    # coche ou décoche toutes les couches présentes dans le treeView
    def precis_global_checkbox_edit(self, check):
        root = self.main.dlg_precis.treeWidget.invisibleRootItem()
        for i in range(root.childCount()):
            signal = root.child(i)
            num_children = signal.childCount()
            signal.setCheckState(0, check)
            if (num_children != 0):
                self.precis_global_checkbox_edit2(num_children, signal, check)
    def precis_global_checkbox_edit2(self, num_children, parent, check):
        for i in range(parent.childCount()):
            child = parent.child(i)
            num_children = child.childCount()
            child.setCheckState(0, check)
            if num_children != 0:
                self.precis_global_checkbox_edit2(num_children, child, check)
    def precis_check_layer_boxes(self):
        self.precis_global_checkbox_edit(QtCore.Qt.Checked)
    def precis_uncheck_layer_boxes(self):
        self.precis_global_checkbox_edit(QtCore.Qt.Unchecked)

    # mets a jour les checkbox pour les couches
    def precis_update_layer_boxes(self):
        temp = self.main.dlg_precis.lineEdit.text()
        if (temp != ""):
            self.main.dlg_precis.lineEdit.setText("")
            self.main.recherche.search_couche()
        root = self.main.dlg_precis.treeWidget.invisibleRootItem()
        for items in self.main.couche_list:
            for i in range(root.childCount()):
                ...
                if (signal.text(0) == items[0] and signal.checkState(0) != items[2]):
                    items[2] = signal.checkState(0)
                if (num_children != 0):
                    self.precis_update_layer_boxes2(num_children, signal)
        self.main.dlg_precis.lineEdit.setText(temp)
        self.main.recherche.search_couche()
    def precis_update_layer_boxes2(self, num_children, parent):
        for items in self.main.couche_list:
            for i in range(parent.childCount()):
                child = parent.child(i)
                num_children = child.childCount()
                if (child.text(0) == items[0] and child.checkState(0) != items[2]):
                    items[2] = child.checkState(0)
                if num_children != 0:
                    self.precis_update_layer_boxes2(num_children, child)


    # coche tous les layers actifs dans la gestion de couches de QGIS
    def precis_check_active_layers(self):
        root = self.main.dlg_precis.treeWidget.invisibleRootItem()
        allLayers = qgis.core.QgsProject.instance().layerTreeRoot().children()
        for layer in allLayers:
            if (layer.isVisible()):
                ...
                if(type(layer) == qgis._core.QgsLayerTreeGroup):
                    for i in range(root.childCount()):
                        ...
                        num_children = signal.childCount()
                        if(layer.name() == signal.text(0)):
                            self.precis_activate_in_groups(num_children, signal, layer)
    def precis_activate_in_groups(self, nb, signal, layer):
        i = 0
        if (nb == 0):
            if (layer.isVisible()):
                signal.setCheckState(0, 2)
        ...
        for childs in node.children():
            child = signal.child(i)
            num_children = child.childCount()
            self.precis_activate_in_groups(num_children, child, childs)
            if layer.name() == child.text(0) and layer.isVisible():
                child.setCheckState(0, 2)
            i += 1
    
    # ajoute les groupes dans la liste couche_list
    def precis_append_for_groups(self, num_children, parent, total):
        for i in range(parent.childCount()):
            child = parent.child(i)
            num_children = child.childCount()
            self.main.couche_list.append([child.text(0), total, child.checkState(0)])
            if num_children != 0:
                self.precis_append_for_groups(num_children, child, total)
            total += 1

    # coche les boites précédemment checkés dans les groupes
    def precis_check_groups(self, num_children, parent):
        for items in self.main.couche_list:
            for i in range(parent.childCount()):
                child = parent.child(i)
                num_children = child.childCount()
                if (child.text(0) == items[0] and child.checkState(0) != items[2]):
                        child.setCheckState(0, items[2])
                if num_children != 0:
                    self.precis_check_groups(num_children, child)

    # créé les groupes et ajoutes les enfants dans les groupes
    def precis_set_group_items(self, item, nb):
        if (item.children() == []):
            return 1
        if (nb == None):
            parent = QTreeWidgetItem(self.main.dlg_precis.treeWidget)
        else:
            parent = QTreeWidgetItem(nb)
        parent.setText(0, '%s' % item.name())
        parent.setFlags(parent.flags() | QtCore.Qt.ItemIsTristate | QtCore.Qt.ItemIsUserCheckable)
        ...
        for childs in node.children():
            if(type(childs) == qgis._core.QgsLayerTreeGroup):
                if (self.precis_set_group_items(childs, parent) == 0):
                    continue
            child = QTreeWidgetItem(parent)
            child.setFlags(child.flags() | QtCore.Qt.ItemIsUserCheckable)
            child.setText(0, '%s' % childs.name())
    ...
    # récupère la taille de l'abrorésence des couches
    def precis_get_tree_size(self):
        allLayers = qgis.core.QgsProject.instance().layerTreeRoot().children()
        if allLayers:
            for i in allLayers:
                if(type(i) == qgis._core.QgsLayerTreeGroup):
                    self.precis_check_if_subgroup(i)
    def precis_check_if_subgroup(self, item):
        if (item.children() == []):
            return 1
        for childs in item.children():
            if(type(childs) == qgis._core.QgsLayerTreeGroup):
                if (self.precis_check_if_subgroup(childs) == 0):
                    self.main.total_sub_groups += 1
                    continue
        return 0
        ...
        for i in allLayers:
            ...
            if(type(i) == qgis._core.QgsLayerTreeLayer):
                if i.name() == self.main.controlpoint_layer_name:
                    ...
                    self.controlpoint_layer = layer
                    continue
                layer = QTreeWidgetItem(self.main.dlg_precis.treeWidget)
                layer.setText(0, '%s' % i.name())
                layer.setCheckState(0, 0)
            if(type(i) == qgis._core.QgsLayerTreeGroup):
